...
if TYPE_CHECKING:
    ...
    from datos.geometry import LipidInstance
...
def export_pdb(
    ...
    lines.append("REMARK  Resolucion espacial: posiciones continuas en Angstrom")
    lines.append(
        "REMARK  Grano de cola: %.2f A/segmento (9 segmentos por cadena)" % (14.5 * 1.26 / 9)
    )
    lines.append(
    ...
    def write_atom(atom_name, res_name, chain, res_num, x, y, z, occupancy=1.0, bfactor=0.0):
        nonlocal atom_idx
        if wrap_periodic:
            x = x % membrane.Lx
            y = y % membrane.Ly
    ...
    for leaflet_lips, chain in [
        ...
        for lip in leaflet_lips:
            ...
            write_atom("HD  ", res_name, chain, res_idx, *lip.head_pos, occ, bf)

            if lip.lipid_type.glyc_offset > 0:
                write_atom("GL  ", res_name, chain, res_idx, *lip.glycerol_pos, occ, bf)

            if lip.tail1:
                for seg_i, pt in enumerate(lip.tail1):
                    aname = "T1%02d" % seg_i if seg_i < 100 else "T1XX"
                    write_atom(aname, res_name, chain, res_idx, *pt, occ, bf)
    ...
    n_atoms = atom_idx - 1
    print(
        "  -> posiciones/pdb/%s  %d atomos  %d residuos"
...
def export_csv_positions(
    ...
    """
    if path is None:
        path = os.path.join(_csv_dir(), "posiciones_simulacion%04d.csv" % membrane.seed)
    ...
    todos = [(lip, i) for i, lip in enumerate(membrane.outer_leaflet)] + [
        (lip, i + len(membrane.outer_leaflet)) for i, lip in enumerate(membrane.inner_leaflet)
    ]
...
def export_polnet_particle_list(
    ...
    """
    if path is None:
        path = os.path.join(_polnet_dir(), "polnet_particulas_simulacion%04d.csv" % membrane.seed)
