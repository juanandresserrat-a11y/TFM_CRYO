...
def export_density_mrc(
    ...
    print(
        "  -> mrc/densidad/%s  %.0fx%.0fx%.0f voxeles  %.0f A/voxel"
        % (
            fname,
            H_norm.shape[0],
            H_norm.shape[1],
...
def export_label_mrc(
    ...
    # CORRECCIÓN: usar el centro real de las cabezas (total_thick/2)
    # en lugar de g.z_outer/g.z_inner que ahora son el glicerol.
    from datos.lipid_types import LIPID_TYPES

    hg_mean = (
        sum(
            LIPID_TYPES[k].hg_thick * f for k, f in membrane.comp_outer.items() if k in LIPID_TYPES
        )
        if membrane.comp_outer
        else 9.0
    )
    head_center_o = g.total_thick / 2.0
    head_center_i = -g.total_thick / 2.0
    hg_half_o = (head_center_o - hg_mean / 2.0) / 10.0
    hg_half_i = (head_center_i + hg_mean / 2.0) / 10.0
...
def generate_polnet_yaml(
    ...
    """
    density_path = os.path.abspath(
        os.path.join(_mrc_densidad_dir(), "bicapa_simulacion%04d.mrc" % membrane.seed)
    )
