...
from simulacion.electron_density import ELECTRON_DENSITY
...
def benchmark_helfrich(membrane: "BicapaCryoET") -> Dict:
    ...
    combined_accuracy = (
        w_slope * acc_slope + w_kc * acc_kc + w_r2 * acc_r2 + w_npts * acc_npts + w_h2 * acc_helf2
    )
...
def benchmark_thickness(membrane: "BicapaCryoET") -> Dict:
    ...
    return {
        ...
        "mean_A": float(vals.mean()) * 10.0,
        "std_nm": float(vals.std()),
        "std_A": float(vals.std()) * 10.0,
        "lo_mean_nm": lo_mean,
        "ld_mean_nm": ld_mean,
...
def benchmark_order_parameter(membrane: "BicapaCryoET") -> Dict:
    """
    Verifica los valores de S_CH para fases ordenada (Lo) y fluida (Ld).
    """
...
def benchmark_raft_correlation(membrane: "BicapaCryoET") -> Dict:
    ...
    if len(acf_radial) > 0 and acf_radial[0] != 0:
        ...
        mx = np.max(np.abs(acf_radial))
        if mx > 0:
            acf_radial = acf_radial / mx
...
def benchmark_electron_density(membrane: "BicapaCryoET") -> Dict:
    ...
    """
    from simulacion.electron_density import electron_density_profile
    ...
    head_zone = ((z_centers >= z_o * 0.5) & (z_centers <= z_o * 1.3)) | (
...
def run_all_benchmarks(membrane: "BicapaCryoET") -> Dict:
    ...
    print(
        "  Resultado: %d/%d pruebas superadas  score: %.3f"
        % (passed, total, mean_accuracy / 100.0)
    )
    for name, res in results.items():
        if "accuracy" in res:
...
def plot_validation_panel(
    ...
    with plt.rc_context(PLT_STYLE):
        ...
        fig.suptitle(
            ...
            "Benchmarks fisicos contra rangos experimentales de cryo-ET" % membrane.seed,
            fontsize=13,
            fontweight="bold",
        )
        ...
        if "t_range" in thick:
            ...
            for pk in thick.get("peaks", []):
                ax2.axvline(pk, color="#e63946", lw=1.5, ls="--", label="%.2f nm" % pk)
            if "diff_A" in thick:
                ax2.text(
                    0.05,
                    0.95,
        ...
        ax2.set_title(
            "Grosor bimodal Lo/Ld [%s]\nPicos Lo y Ld separados 4-8 A" % status_str,
            fontsize=9,
            fontweight="bold",
            color=status_color,
        )
        ...
        if "r_vals" in raft:
            ...
            ax4.plot(
                ...
                color="#e63946",
                lw=1.8,
                label="Ajuste xi=%.1f nm" % xi,
            )
            ax4.axhline(1 / np.e, color="#333333", lw=0.8, ls=":", alpha=0.6, label="1/e")
        ...
        bars = ax5.bar(cats, vals_idig, color=colors_idig, edgecolor="#333333", lw=0.8, width=0.5)
        ...
        ax5.set_title(
            "Interdigitacion Lo > Ld [%s]\nChaisson et al. 2025" % status_str,
            fontsize=9,
            fontweight="bold",
            color=status_color,
        )
        ...
        if "z_centers" in ed_res:
            ...
            ax6.fill_betweenx(z_c / 10.0, 0.334, ed_p, alpha=0.3, color="#d4a017")
            ax6.axhline(
                membrane.geometry.z_outer / 10.0,
                color="#2dc653",
                ls="--",
                lw=1.2,
                label="Cabezas ext",
            ...
            ax6.axhline(
                membrane.geometry.z_inner / 10.0,
                color="#e63946",
                ls="--",
                lw=1.2,
                label="Cabezas int",
            ...
            for lo, hi, col, lbl in [
                (
                    BENCHMARKS["ed_head_peak_e_A3"][0],
                    BENCHMARKS["ed_head_peak_e_A3"][1],
                    "#2dc653",
                    "Ref cabezas (pico)",
                ),
                (
                    BENCHMARKS["ed_tail_e_A3"][0],
                    BENCHMARKS["ed_tail_e_A3"][1],
                    "#3a86ff",
                    "Ref colas",
                ),
            ]:
        ...
        ax6.set_title(
            "Perfil densidad electronica [%s]\nPatron dark-bright-dark" % status_str,
            fontsize=9,
            fontweight="bold",
            color=status_color,
        )
...
def save_benchmark_json(results: Dict, membrane: "BicapaCryoET"):
    ...
    for k, v in results.items():
        if not isinstance(v, dict):
            continue
        clean[k] = {kk: _to_native(vv) for kk, vv in v.items() if kk not in RAW_KEYS}
