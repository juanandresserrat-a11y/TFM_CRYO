...
def pip_density_map(
    ...
    for lip in membrane.inner_leaflet:
        ...
        ix = min(int(lip.head_pos[0] / membrane.Lx * bins), bins - 1)
        iy = min(int(lip.head_pos[1] / membrane.Ly * bins), bins - 1)
        H[ix, iy] += (1 + lip.lipid_type.pip_order * 0.5) * lip.lipid_type.mass
    return gaussian_filter(H / ab, sigma=sigma)
...
def interdigitation_map(
    ...
    R = raft_fraction_map(membrane, membrane.outer_leaflet + membrane.inner_leaflet, bins=bins)
...
def xz_projection(
    ...
    ms = np.array(
        [
            ...
            * (
                1.5
                if abs(l.head_pos[2] / 10.0 - (head_z_ext if l.leaflet == "sup" else head_z_int))
                < 0.5
                else 0.5
            )
            for l in todos
