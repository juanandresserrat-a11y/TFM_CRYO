...
Produce seis paneles de publicación independientes que cubren todas las
secciones de resultados habituales en un TFM de biofísica computacional:

  R1  Caracterización biofísica de la bicapa
      Composición lipídica, parámetros elásticos, perfil de densidad electrónica
      y distribución de grosor — la "ficha técnica" de cada semilla.
  ...
  R5  Galería de canales de training
      ...
      visual — figura de referencia para la sección de ML.

  R6  Calidad de la simulación cryo-ET
      Comparación imagen limpia / CTF / ruido + espectro de potencia
      radialmente promediado — justifica el realismo del pipeline de imagen.
...
Salida:
    CryoET/resultados/
        R1_caracterizacion_seed{N}.pdf
        R2_validacion_seed{N}.pdf
        R3_organizacion_lateral_seed{N}.pdf
        R4_comparativa_multisemilla.pdf
        R5_canales_training_seed{N}.pdf
        R6_calidad_cryoET_seed{N}.pdf
"""
...
# Paleta coherente con las figuras de publicación del proyecto
C = {
    "lo":      "#b89a68",   # Lo / gel — beige ocre
    "ld":      "#a8c8e0",   # Ld / fluido — azul
    "pip":     "#c0392b",   # PIPs
...
def _save(fig: plt.Figure, name: str, dpi: int = 300) -> str:
    path = os.path.join(_results_dir(), name + ".pdf")
...
def plot_R1_caracterizacion(membrane: BicapaCryoET, dpi: int = 300) -> str:
    """
    Panel de caracterización biofísica completa de una semilla.

    Incluye:
    A  Composición lipídica de ambas monocapas (barras horizontales apiladas)
    B  Parámetros elásticos kc y σ con rangos experimentales de referencia
    C  Perfil de densidad electrónica 1D (dark-bright-dark)
    D  Distribución de grosor local D_PP con desglose Lo/Ld
    """
    seed = membrane.seed
    g = membrane.geometry
    ...
    with plt.rc_context(PUB_RC):
        ...
        # ── C: Perfil de densidad electrónica ──────────────────────────────
        ...
        ax_thick.set_xlabel("Grosor $D_{PP}$ (Å)")
        ax_thick.set_ylabel("Densidad de prob. (Å⁻¹)")
        ax_thick.set_title("D  Distribución de grosor Lo / Ld")
        ax_thick.legend(fontsize=7)
        _despine(ax_thick)

        return _save(fig, f"R1_caracterizacion_seed{seed:04d}", dpi)


# ══════════════════════════════════════════════════════════════════════════════
# R2 — Validación cuantitativa
# ══════════════════════════════════════════════════════════════════════════════
...
def plot_R2_validacion(membrane: BicapaCryoET,
                       ...
                       dpi: int = 300) -> str:
    """
    Panel de validación biofísica: tabla de benchmarks + detalle gráfico.

    A  Tabla resumen: métrica medida vs. rango de referencia bibliográfica
    B  Barras de éxito por benchmark (pass/fail con margen)
    C  Espectro de Helfrich q⁴ vs. <|h_q|²>
    D  Histogramas S_CH gel / fluido
    E  Perfil de densidad electrónica vs. valores Nagle 2000
    F  ACF radial de dominios Lo
    """
    seed = membrane.seed
    if results is None:
    ...
    with plt.rc_context(PUB_RC):
        ...
        fig.suptitle(
            ...
            f"[Score: {results['summary']['score']*100:.0f}% "
            f"({results['summary']['passed']}/{results['summary']['total']} benchmarks)]",
            fontsize=11, fontweight="bold", y=0.98)
        ...
        rows = [
            ("Helfrich pendiente",  "slope_high_q", "helfrich",
             "−4 ± 0.3", results["helfrich"].get("pass", False)),
            ("Grosor D_PP (Å)",     "mean_nm",       "thickness",
             "35–50",     results["thickness"].get("pass_diff", False)),
            ("ΔD Lo−Ld (Å)",        "diff_A",        "thickness",
             "3–6",       results["thickness"].get("pass_diff", False)),
            ("S_CH gel",           "gel_mean",      "order_parameter",
             "0.85–0.95", results["order_parameter"].get("pass_gel", False)),
            ("S_CH fluido",        "fluid_mean",    "order_parameter",
             "0.60–0.75", results["order_parameter"].get("pass_fluid", False)),
            ("Long. corr. Lo (nm)","xi_nm",         "raft_correlation",
             "5–25",      results["raft_correlation"].get("pass", False)),
            ("Interdig. Lo > Ld",  "lo_gt_ld",      "interdigitation",
             "True",      results["interdigitation"].get("pass", False)),
            ("ED cabeza (e·Å⁻³)","ed_head_peak",   "electron_density",
             "0.44–0.50", results["electron_density"].get("pass_head", False)),
            ("ED cola (e·Å⁻³)", "ed_tail",         "electron_density",
             "0.28–0.31", results["electron_density"].get("pass_tail", False)),
        ]
        ax_tbl.axis("off")
        col_labels = ["Parámetro", "Valor medido", "Ref. bibliográfica", ""]
        col_widths = [0.38, 0.25, 0.28, 0.09]
        x_positions = [0.0]
        ...
        for j, (lbl, x) in enumerate(zip(col_labels, x_positions)):
            ax_tbl.text(x, 1.05, lbl, transform=ax_tbl.transAxes,
                        fontsize=8, fontweight="bold", va="top",
                        color=C["line"])

        # Separador
        ax_tbl.axhline(1.0, color=C["line"], lw=0.8,
                       transform=ax_tbl.transAxes, xmin=0, xmax=1)

        n = len(rows)
        for i, (label, key, bench, ref, passed) in enumerate(rows):
            y = 1.0 - (i + 1) * (1.0 / (n + 1))
            val = results[bench].get(key, "—")
            ...
            bg_col = "#f6fff8" if passed else "#fff5f5"
            ax_tbl.axhspan(y - 0.04, y + 0.04, color=bg_col, alpha=0.7,
                           transform=ax_tbl.transAxes)
            ax_tbl.text(x_positions[0], y, label, transform=ax_tbl.transAxes,
                        fontsize=7.5, va="center")
            ax_tbl.text(x_positions[1], y, val_str, transform=ax_tbl.transAxes,
                        fontsize=7.5, va="center", color=C["line"])
            ax_tbl.text(x_positions[2], y, ref, transform=ax_tbl.transAxes,
                        fontsize=7, va="center", color=C["neutral"])
            status_col = C["pass"] if passed else C["fail"]
            ax_tbl.text(x_positions[3], y, "✓" if passed else "✗",
                        transform=ax_tbl.transAxes, fontsize=9,
                        va="center", color=status_col, fontweight="bold")

        ax_tbl.set_xlim(0, 1)
        ax_tbl.set_ylim(0, 1.15)
        ax_tbl.set_title("A  Tabla de benchmarks biofísicos")

        # ── B: Barras de éxito ──────────────────────────────────────────────
        _panel_label(ax_bar, "B")
        bench_names = ["Helfrich", "Grosor", "Orden\nS_CH",
                       "Corr.\nLo", "Interdig.", "Densidad\nelec."]
        bench_keys  = ["helfrich", "thickness", "order_parameter",
                       "raft_correlation", "interdigitation", "electron_density"]
        scores = []
        for bk in bench_keys:
            b = results.get(bk, {})
            passed_sub = [v for k, v in b.items()
                          if k.startswith("pass") and isinstance(v, bool)]
            scores.append(sum(passed_sub) / max(len(passed_sub), 1))

        bar_cols = [C["pass"] if s >= 0.5 else C["fail"] for s in scores]
        ...
        ax_helf.set_xlabel("$q$ (nm⁻¹)")
        ax_helf.set_ylabel(r"$\langle|\hat{h}_q|^2\rangle$ (nm²)")
        ...
        ax_sch.set_xlabel("$S_{CH}$")
        ax_sch.set_ylabel("PDF")
        ax_sch.set_title("D  Parámetro de orden $S_{CH}$")
        ax_sch.legend(fontsize=6.5)
        _despine(ax_sch)

        # ── E: Perfil ED vs. referencia ─────────────────────────────────────
        ...
        return _save(fig, f"R2_validacion_seed{seed:04d}", dpi)


# ══════════════════════════════════════════════════════════════════════════════
# R3 — Organización lateral
# ══════════════════════════════════════════════════════════════════════════════

def plot_R3_organizacion(membrane: BicapaCryoET, dpi: int = 300) -> str:
    """
    Organización lateral de la bicapa: fases, PIPs y clustering.

    A  Mapa 2D Lo/Ld con gradiente suave y contorno de dominio
    B  Distribución espacial de PIPs por especie sobre el mapa de fase
    C  Mapa 2D de parámetro de orden S_CH
    D  Mapa 2D de densidad de PIPs (Da·Å⁻²)
    """
    seed = membrane.seed
    Lx, Ly = membrane.Lx / 10, membrane.Ly / 10
    ext = [0, Lx, 0, Ly]

    raft_map = analysis.raft_fraction_map(
        membrane, membrane.outer_leaflet + membrane.inner_leaflet,
        bins=160, sigma=2.0)
    ...
    cmap_lo_ld = mcolors.LinearSegmentedColormap.from_list(
        "lo_ld", [(0.0, "#a8c8e0"), (0.45, "#ccd8d0"),
                  (0.55, "#d6cfc0"), (1.0, "#b89a68")], N=256)
    ...
    with plt.rc_context(PUB_RC):
        ...
        # A: Mapa de fase Lo/Ld
        _panel_label(axes[0], "A")
        im0 = axes[0].imshow(raft_map.T, origin="lower", extent=ext,
                             cmap=cmap_lo_ld, vmin=0, vmax=1,
                             aspect="equal", interpolation="bilinear")
        xg = np.linspace(0, Lx, raft_map.shape[0])
        yg = np.linspace(0, Ly, raft_map.shape[1])
        axes[0].contour(xg, yg, raft_map.T, levels=[0.5],
                        colors=[C["line"]], linewidths=1.0,
        ...
        cb0.set_ticklabels(["Ld", "0.5", "Lo"], fontsize=6.5)
        cb0.ax.tick_params(labelsize=6)
        lo_pct = (raft_map > 0.5).mean() * 100
        axes[0].text(0.03, 0.97,
                     f"Lo: {lo_pct:.1f}%  |  "
                     f"{len(membrane.rafts_outer)+len(membrane.rafts_inner)} dominios",
                     transform=axes[0].transAxes, fontsize=7,
                     va="top", bbox=dict(fc="white", ec=C["neutral"],
                                        lw=0.5, pad=2.5, alpha=0.9))
        axes[0].set_title("A  Fracción de fase Lo local")
        axes[0].set_xlabel("$x$ (nm)")
        axes[0].set_ylabel("$y$ (nm)")

        # B: PIPs sobre mapa de fase
        _panel_label(axes[1], "B")
        axes[1].imshow(raft_map.T, origin="lower", extent=ext,
                       cmap=cmap_lo_ld, vmin=0, vmax=1,
                       aspect="equal", interpolation="bilinear", alpha=0.6)
        axes[1].contour(xg, yg, raft_map.T, levels=[0.5],
                        colors=[C["line"]], linewidths=0.9,
        ...
        for sp in sorted(PIP_CLR.keys()):
            ...
            xs, ys = zip(*pts)
            col = PIP_CLR[sp]
            axes[1].scatter(xs, ys, s=10, c=col, marker="o",
            ...
            n_total_pip += len(pts)
        if pip_handles:
            axes[1].legend(handles=pip_handles, loc="upper right",
                           fontsize=6.5, frameon=True)
        axes[1].text(0.03, 0.03, f"{n_total_pip} PIPs total",
                     transform=axes[1].transAxes, fontsize=7,
                     bbox=dict(fc="white", ec=C["neutral"], lw=0.5,
                               pad=2, alpha=0.9))
        axes[1].set_title("B  PIPs sobre mapa de fase Lo/Ld")
        axes[1].set_xlabel("$x$ (nm)")
        axes[1].set_ylabel("$y$ (nm)")

        # C: Parámetro de orden S_CH
        _panel_label(axes[2], "C")
        im2 = axes[2].imshow(order_map.T, origin="lower", extent=ext,
                             cmap="RdYlGn", vmin=0.55, vmax=0.98,
                             aspect="equal", interpolation="bilinear")
        cb2 = plt.colorbar(im2, ax=axes[2], shrink=0.85, pad=0.02)
        cb2.set_label("$S_{CH}$", fontsize=7, labelpad=3)
        cb2.ax.tick_params(labelsize=6)
        axes[2].set_title("C  Parámetro de orden $S_{CH}$\n[Piggot 2017]")
        axes[2].set_xlabel("$x$ (nm)")
        axes[2].set_ylabel("$y$ (nm)")

        # D: Densidad de PIPs
        _panel_label(axes[3], "D")
        im3 = axes[3].imshow(pip_map.T, origin="lower", extent=ext,
                             cmap="hot_r", aspect="equal",
                             interpolation="bilinear")
        cb3 = plt.colorbar(im3, ax=axes[3], shrink=0.85, pad=0.02)
        cb3.set_label("Densidad PIP (Da·Å⁻²)", fontsize=7, labelpad=3)
        cb3.ax.tick_params(labelsize=6)
        # Marcar clusters
        for cl in getattr(membrane, "pip_clusters", []):
            cx, cy = cl["center"][0]/10, cl["center"][1]/10
            r = cl.get("radius", 5) / 10
            circ = plt.Circle((cx, cy), r, fill=False,
                              edgecolor="cyan", linewidth=1.0,
                              linestyle=":", alpha=0.8)
            axes[3].add_patch(circ)
        axes[3].set_title("D  Densidad de PIPs\n[Di Paolo & De Camilli 2006]")
        axes[3].set_xlabel("$x$ (nm)")
        axes[3].set_ylabel("$y$ (nm)")

        return _save(fig, f"R3_organizacion_lateral_seed{seed:04d}", dpi)
...
def plot_R4_multisemilla(stats: Dict, dpi: int = 300) -> str:
    """
    Variabilidad del dataset: violinplots de propiedades físicas sobre N semillas.

    A  kc (kBT·nm²) — rígidez de membrana
    B  Grosor D_PP (Å) — grosor de la bicapa
    C  S_CH por fase — orden de las cadenas
    D  Score de validación (%) — calidad biofísica
    E  Composición media ± DE — diversidad del dataset
    F  Número de balsas y clusters PIP
    """
    ...
    with plt.rc_context(PUB_RC):
        ...
        _violin(axes[0], kc, C["lo"], "kc",
                ref_lo=20, ref_hi=45)
        axes[0].set_ylabel("$\\kappa_c$ (kBT·nm²)")
        axes[0].set_title("A  Módulo de curvatura $\\kappa_c$")

        # B: Grosor
        _panel_label(axes[1], "B")
        _violin(axes[1], thick, C["ld"], "D_PP",
                ref_lo=35, ref_hi=50)
        axes[1].set_ylabel("$D_{PP}$ (Å)")
        axes[1].set_title("B  Grosor de bicapa $D_{PP}$")
        ...
        _panel_label(axes[3], "D")
        val_pct = val_s * 100
        bar_cols = [C["pass"] if v >= 80 else C["neutral"] if v >= 60 else C["fail"]
                    for v in val_pct]
        ...
        # E: Composición media ± DE (monocapa externa)
        ...
        return _save(fig, "R4_comparativa_multisemilla", dpi)
...
def plot_R5_canales(membrane: BicapaCryoET, dpi: int = 300) -> str:
    """
    Galería de los 12 canales de training en una cuadrícula 3×4.
    Cada canal se normaliza individualmente para maximizar el contraste visual.
    """
    seed = membrane.seed
    bins = 64
    ...
    channels = {
        ...
        "c9  Orden $S_{CH}$":      analysis.order_parameter_map(membrane, bins=bins),
    ...
    with plt.rc_context(PUB_RC):
        ...
        for ax, (title, arr) in zip(axes.ravel(), channels.items()):
            ...
            # Normalización robusta al percentil 2–98
        ...
        return _save(fig, f"R5_canales_training_seed{seed:04d}", dpi)
...
def plot_R6_cryoET(membrane: BicapaCryoET, dpi: int = 300) -> str:
    """
    Calidad del pipeline de imagen cryo-ET simulado.

    A  Imagen limpia (sin CTF, sin ruido)
    B  Imagen con CTF aplicado
    C  Imagen con CTF + ruido realista (SNR ≈ 0.10)
    D  Espectro de potencia radialmente promediado de A, B, C
    E  Curvas CTF para tres valores de defoco típicos
    F  Perfil XZ de densidad electrónica 2D
    """
    seed = membrane.seed
    bins = 90
    ...
    with plt.rc_context(PUB_RC):
        ...
        def _radial_psd(img):
            ...
            q = np.arange(r_max) / (r_max * pixel_A / 10)  # nm⁻¹
            return q[1:], radial[1:]
        ...
        q_max = 0.5 / pixel_A * 10  # nm⁻¹
        q = np.linspace(0.001, q_max, 300)
        Fx = (q / 10).reshape(-1, 1)
        Fy = np.zeros_like(Fx)
        defoci = [1.0, 2.0, 3.5]
        ...
        for df, col in zip(defoci, ctf_colors):
            ...
            ax_curves.plot(q, ctf_1d, lw=1.4, color=col, label=f"Δf = {df} μm")
        ax_curves.axhline(0, color=C["neutral"], lw=0.7, ls="--", alpha=0.5)
        ax_curves.set_xlabel("Frecuencia espacial (nm⁻¹)")
        ...
        return _save(fig, f"R6_calidad_cryoET_seed{seed:04d}", dpi)
  ...
  python results.py --sims 1 --dpi 300 --size 50 50
    ...
    if "R4" in only and len(stats_records) >= 2:
        ...
        stats = {
            ...
            "val_scores": [r["val_score"] for r in stats_records],
        }
        plot_R4_multisemilla(stats, dpi=args.dpi)
    elif "R4" in only:
        print("  R4 requiere ≥2 semillas, saltando.")

    print(f"\n✓ Listo. PDFs en: {RESULTS_DIR}")
