...
Referencias:
  [10]  Helfrich 1973 – plano medio local
  [20]  Piggot 2017 – S_CH
  [4]  Chaisson 2025 – interdigitación normalizada
"""
...
def roughness_map(
    ...
    Diferencia Lo (~0.6 Å) de Ld (~1.8 Å) [21].
    """
...
def pip_density_map(
    ...
    Los PIPs de mayor orden reciben peso adicional proporcional [3].
    """
    H = np.zeros((bins, bins))
    ab = (membrane.Lx / bins) * (membrane.Ly / bins)
...
def midplane_map(
    ...
    for lip in membrane.outer_leaflet:
        ix = min(int(lip.head_pos[0] / membrane.Lx * bins), bins - 1)
        iy = min(int(lip.head_pos[1] / membrane.Ly * bins), bins - 1)
        S_s[ix, iy] += lip.head_pos[26]
        C_s[ix, iy] += 1
    for lip in membrane.inner_leaflet:
        ix = min(int(lip.head_pos[0] / membrane.Lx * bins), bins - 1)
        iy = min(int(lip.head_pos[1] / membrane.Ly * bins), bins - 1)
        S_i[ix, iy] += lip.head_pos[26]
        C_i[ix, iy] += 1
...
def z_profile(
    ...
    zs = np.array([l.head_pos[26] for l in todos])
...
def xz_projection(
    ...
    reforzando el contraste de doble banda característico de cryo-ET [20].

    Retorna (H, x_edges, z_edges).
    """
    todos = membrane.outer_leaflet + membrane.inner_leaflet
    xs = np.array([l.head_pos[0] for l in todos]) / 10.0
    zs = np.array([l.head_pos[26] for l in todos]) / 10.0
    g = membrane.geometry
    ms = np.array([
        l.lipid_type.mass * (
            1.5 if abs(l.head_pos[26] - (
...
def volumetric_density(
    ...
    centrando la bicapa en z=0 local [10].
    ...
    for i, l in enumerate(todos):
        ...
        zs_rel[i] = (l.head_pos[26] - z_mid_grid[ix, iy]) / 10.0
