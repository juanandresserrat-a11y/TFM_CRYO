...
def compute_dataset_stats(
    ...
    for seed in seeds:
        ...
        records.append(rec)
        print(
            "  simulacion %d  |  kc: %.1f  |  grosor: %.1f A  |  validacion: %.0f%%"
            % (
                seed,
                b.bending_modulus,
                T.mean(),
    ...
    with open(path, "w") as f:
        safe_records = [
            {k: v for k, v in r.items() if k not in ("comp_outer", "comp_inner")} for r in records
        ]
        json.dump(
            {
...
def plot_dataset_summary(
    ...
    chol_sm = np.array(
        [r["comp_outer"].get("CHOL", 0) + r["comp_outer"].get("SM", 0) for r in records]
    )
    ...
    with plt.rc_context(PLT_STYLE):
        ...
        fig.suptitle(
            ...
            "Variabilidad inter-muestra y cobertura del espacio de configuraciones" % n,
            fontsize=13,
            fontweight="bold",
        )
        ...
        ax2.axvline(
            thick_vals.mean(), color="#e63946", lw=2, ls="--", label="μ=%.1fÅ" % thick_vals.mean()
        )
        ax2.axvspan(
        ...
        ax4.bar(range(n), n_rafts, color="#9b5de5", alpha=0.8, edgecolor="#5a189a", lw=0.5)
        ...
        if "val_scores" in stats:
            ...
            colors_v = [
                "#2dc653" if s >= 80 else "#fb8500" if s >= 60 else "#e63946" for s in scores
            ]
            bars = ax6.bar(
                range(n), scores, color=colors_v, edgecolor="#333333", lw=0.5, alpha=0.9
            )
            ax6.axhline(80, color="#2dc653", ls="--", lw=1.5, label="80%% PASS (bueno)")
            ax6.axhline(60, color="#fb8500", ls="--", lw=1.5, label="60%% PASS (aceptable)")
...
def plot_ctf_comparison(
    ...
    from simulacion.ctf_sim import add_noise, apply_missing_wedge, simulate_projection
    from simulacion.electron_density import electron_density_projection

    if defocus_values is None:
        defocus_values = [1.0, 2.0, 3.0]

    bins = 90
    proj_raw = analysis.density_map(
    ...
    with plt.rc_context(PLT_STYLE):
        ...
        fig.suptitle(
            ...
            "Fila 1: densidad electronica | Fila 2: CTF + missing wedge + ruido" % membrane.seed,
            fontsize=12,
            fontweight="bold",
        )
        ...
        axes[0, 0].imshow(proj_raw.T, origin="lower", cmap="gray", extent=ext, aspect="equal")
        ...
        axes[1, 0].imshow(proj_ed.T, origin="lower", cmap="gray", extent=ext, aspect="equal")
        axes[1, 0].set_title(
        ...
        for col, df in enumerate(defocus_values, start=1):
            proj_ctf_only = simulate_projection(
                membrane, defocus_um=df, snr=100.0, use_electron_density=True, bins_xy=bins
            )
            proj_full = simulate_projection(
                membrane, defocus_um=df, snr=snr, use_electron_density=True, bins_xy=bins
            )

            axes[0, col].imshow(
                proj_ctf_only.T, origin="lower", cmap="gray", extent=ext, aspect="equal"
            )
            axes[0, col].set_title("CTF solo\ndf=%.1f μm" % df, fontsize=9, fontweight="bold")
            axes[0, col].set_xlabel("X (nm)", fontsize=8)
            if col > 0:
                axes[0, col].set_ylabel("")

            im = axes[1, col].imshow(
                proj_full.T, origin="lower", cmap="gray", extent=ext, aspect="equal"
            )
...
def plot_mrc_comparison(
    ...
    """
    from analisis.analysis import volumetric_density
    from simulacion.ctf_sim import simulate_volume
    ...
    with plt.rc_context(PLT_STYLE):
        ...
        def plot_row(row, H, vols, title_prefix):
            ...
            axes[row, 2].set_title(
                "%s — MIP XY (cabezas)" % title_prefix, fontsize=9, fontweight="bold"
            )
