...
def _composition(membrane):
    ...
    return (
        {s: outer.get(s, 0) / n_out for s in ALL_SPECIES},
...
def _thickness_dist(membrane):
    from analisis.analysis import thickness_map
...
def _scalebar(ax, Lx, Ly, length_nm=10.0, color="#333333", lw=2.0, pos_frac=(0.05, 0.05)):
    px = Lx * pos_frac[0]
    py = Ly * pos_frac[1]
    ax.plot(
        [px, px + length_nm],
    ...
    ax.plot([px, px + length_nm], [py, py], color=color, lw=lw, solid_capstyle="butt", zorder=10)
    ax.text(
        px + length_nm / 2,
        py + Ly * 0.024,
        "%.0f nm" % length_nm,
...
def _styled_colorbar(
    fig, im, ax, label, ticks=None, ticklabels=None, shrink=0.88, pad=0.02, aspect=20
):
...
def _info_box(ax, text, loc="upper left", fs=7.5):
    ...
    ax.text(
        ...
        linespacing=1.45,
        color="#222222",
        bbox=dict(boxstyle="round,pad=0.38", fc="white", ec="#bbbbbb", alpha=0.92, lw=0.6),
        zorder=15,
    )
...
def plot_fig1_perfil_ED(membrane, vol, stats, dpi=300):
    ...
    # Posiciones de las CABEZAS (no del glicerol)
    ...
    with plt.rc_context(PUB_RC):
        ...
        ax.plot(ed_prof, z_nm, color="#2c3e50", lw=2.2, zorder=5, label="ED media del parche")
        ax.fill_betweenx(
            z_nm,
            0.334,
            ed_prof,
        ...
        ax.fill_betweenx(
            ...
            alpha=0.25,
            color=C["tail"],
            label="Núcleo hidrofóbico",
        )
        ax.axvline(
            0.334, color="#888888", lw=1.0, ls=":", label="Agua (0.334 $e \\cdot \\AA^{-3}$)"
        )
        ax.axhline(
            z_head_ext_nm,
            color=C["head"],
            ...
            ls="--",
            alpha=0.8,
            label="Cabezas ext. z=%.2f nm" % z_head_ext_nm,
        )
        ax.axhline(
            z_head_int_nm,
            color="#e67e22",
            ...
            ls="--",
            alpha=0.8,
            label="Cabezas int. z=%.2f nm" % z_head_int_nm,
        )
        ax.axhline(0, color="#aaaaaa", lw=0.7, ls=":", alpha=0.6)

        ax.annotate(
            "",
            xy=(0.51, z_head_ext_nm),
            xytext=(0.51, z_head_int_nm),
            arrowprops=dict(arrowstyle="<->", color="#555555", lw=1.2),
        )
        ax.text(
            0.52, 0.0, "D$_{HH}$=%.1f Å" % g.total_thick, fontsize=9, color="#555555", va="center"
        )

        ax.set_xlabel("Densidad electrónica ($e \\cdot \\AA^{-3}$)")
        ax.set_ylabel("Posición axial Z (nm)")
        ax.set_title(
            "Perfil 1D de densidad electrónica\n"
        ...
        path = os.path.join(_sim_fig_dir(seed), "fig1_sim%04d_perfil_ED.png" % seed)
...
def plot_fig2_composicion(membrane, dpi=300):
    ...
    pip_spc = Counter(l.lipid_type.name for l in membrane.inner_leaflet if l.is_pip)
    ...
    with plt.rc_context(PUB_RC):
        ...
        sp_plot = [
            s for s in ALL_SPECIES if out_frac.get(s, 0) > 0.004 or in_frac.get(s, 0) > 0.004
        ]
        ...
        gs_r = gridspec.GridSpecFromSubplotSpec(
            2, 1, subplot_spec=gs[0, 1], height_ratios=[1.6, 1.0], hspace=0.08
        )
        ...
        if pip_spc:
            ...
            ax2.set_title("Fosfoinosítidos (PIPs)\nmonocapa interna", loc="left", pad=5)

            ax_tbl.axis("off")
            try:
                from datos.lipid_types import LIPID_TYPES
                    ...
                    if lt:
                        rows_tbl.append(
                            [
                                ...
                                "%+d" % lt.charge,
                                "%.1f%%"
                                % (100 * pip_spc[sp] / max(len(membrane.inner_leaflet), 1)),
                            ]
                        )
                if rows_tbl:
                    tbl = ax_tbl.table(
                        cellText=rows_tbl, colLabels=col_lbls_t, loc="center", cellLoc="center"
                    )
        ...
        path = os.path.join(_sim_fig_dir(seed), "fig2_sim%04d_composicion.png" % seed)
...
def plot_fig3_helfrich(membrane, dpi=300):
    ...
    with plt.rc_context(PUB_RC):
        ...
        if len(q_nm) > 4:
            ...
            mask_fit = q_nm > 0.12
            if mask_fit.sum() > 3:
                coef = np.polyfit(np.log(q_nm[mask_fit]), np.log(p_helf[mask_fit]), 1)
        ...
        path = os.path.join(_sim_fig_dir(seed), "fig3_sim%04d_helfrich.png" % seed)
        fig.savefig(path, dpi=dpi)
        plt.close(fig)
        print("  -> %s" % path)
    return path
...
def plot_fig4_grosor(membrane, stats, dpi=300):
    ...
    with plt.rc_context(PUB_RC):
        ...
        if len(thick_A) > 10:
            ...
            peaks, _ = find_peaks(
                y_kde, distance=int(len(kde_x) / 12), prominence=y_kde.max() * 0.05
            )
            ...
            if len(peaks) >= 2:
                ...
                _info_box(
                    ax, "$\\Delta D_{Lo-Ld}$ = %.1f Å\n(exp: 4–8 Å)" % delta, loc="upper right"
                )
        ...
        fig.tight_layout(rect=[0, 0.03, 1, 1])
        path = os.path.join(_sim_fig_dir(seed), "fig4_sim%04d_grosor.png" % seed)
        fig.savefig(path, dpi=dpi)
        plt.close(fig)
        print("  -> %s" % path)
    return path
...
def plot_fig5_mapa_raft(membrane, dpi=300):
    ...
    n_rafts = len(getattr(membrane, "rafts_outer", [])) + len(getattr(membrane, "rafts_inner", []))
    ...
    with plt.rc_context(PUB_RC):
        ...
        ax.set_xlabel("$x$ (nm)")
        ax.set_ylabel("$y$ (nm)")
        ax.set_title("Fracción Lo — monocapa externa\n– – –  límite Lo/Ld (0.50)", pad=6)
        _info_box(
            ax, "%d dominio(s) Lo\nFracc. Lo = %.1f%%" % (n_rafts, lo_frac * 100), loc="upper left"
        )

        fig.tight_layout(rect=[0, 0.03, 1, 1])
        path = os.path.join(_sim_fig_dir(seed), "fig5_sim%04d_mapa_raft.png" % seed)
        fig.savefig(path, dpi=dpi)
        plt.close(fig)
        print("  -> %s" % path)
    return path
...
def plot_fig6_mapa_order(membrane, dpi=300):
    ...
    cmap_ord = mcolors.LinearSegmentedColormap.from_list(
        "order_s", ["#2c6e8a", "#a8cfe0", "#f0f0ea", "#5a9e6e", "#1b5e3b"], N=256
    )
    ...
    with plt.rc_context(PUB_RC):
        ...
        if gel_s:
            ...
            axins.fill_between(kde_x, 0, y_kde, color="#1b5e3b", alpha=0.30)
            axins.plot(kde_x, y_kde, color="#1b5e3b", lw=1.3, label="gel/Lo")
        if fluid_s:
            y_kde = gaussian_kde(fluid_s, bw_method=0.10)(kde_x)
            axins.fill_between(kde_x, 0, y_kde, color="#2c6e8a", alpha=0.30)
        ...
        axins.legend(
            fontsize=5.5, loc="upper left", framealpha=0.9, edgecolor="#cccccc", borderpad=0.3
        )

        fig.tight_layout(rect=[0, 0.03, 1, 1])
        path = os.path.join(_sim_fig_dir(seed), "fig6_sim%04d_mapa_order.png" % seed)
        fig.savefig(path, dpi=dpi)
        plt.close(fig)
        print("  -> %s" % path)
    return path
...
def plot_fig7_pip_radial(membrane, dpi=300):
    ...
    with plt.rc_context(PUB_RC):
        ...
        if n_pips > 5 and cx_all:
            ...
            for sp, pos_list in sorted(pip_by_sp.items(), key=lambda kv: -len(kv[1])):
                r_sp = np.array([np.sqrt((x - cx_g) ** 2 + (y - cy_g) ** 2) for x, y in pos_list])
        ...
        fig.tight_layout(rect=[0, 0.03, 1, 1])
        path = os.path.join(_sim_fig_dir(seed), "fig7_sim%04d_pip_radial.png" % seed)
        fig.savefig(path, dpi=dpi)
        plt.close(fig)
        print("  -> %s" % path)
    return path
...
def plot_fig8_parametros(membrane, stats, dpi=300):
    ...
    ROWS = [
        ...
        (
            "GEOMETRÍA",
            "Grosor hidrofóbico $D_C$",
            "%.1f" % g.hydro_thick,
            "26 – 34",
            "Å",
            "Kučerka 2011",
        ),
        ("GEOMETRÍA", "$z$ cabezas ext.", "%.2f" % z_head_ext_nm, "—", "nm", "—"),
        ("GEOMETRÍA", "$z$ cabezas int.", "%.2f" % z_head_int_nm, "—", "nm", "—"),
        (
            "ORDEN",
            "$S_{CH}$ gel (Lo)",
        ...
        (
            "DENS. ELECTR.",
            "Contraste Lo – Ld",
            "%.4f" % stats["contrast_Lo_Ld"],
            "0.008 – 0.020",
            "e·Å$^{-3}$",
            "Chaisson 2025",
        ),
        ("COMPOSICIÓN", "Lípidos totales", "%d" % len(all_lips), "—", "—", "—"),
        (
            "COMPOSICIÓN",
            "PIPs monocapa int.",
    ...
    with plt.rc_context(PUB_RC):
        ...
        col_labels = ["Sección", "Parámetro", "Simulación", "Rango exp.", "Unidad", "Referencia"]
        ...
        for ri, (sec, param, val, rng, unit, ref) in enumerate(ROWS):
            ...
            cells = [sec if sec != prev_sec else "", param, val, rng, unit, ref]
            for ci, (cell, cx, cw) in enumerate(zip(cells, col_x, col_w)):
                bg = (
                    SEC_COLORS.get(sec, "#f5f5f5")
                    if ci == 0
                    else ("#f6f6f6" if ri % 2 == 0 else "white")
                )
                rect = mpatches.FancyBboxPatch(
                    (cx, y_pos - row_h * 0.44),
                    cw,
                ...
                fc = "#1a3a5c" if ci == 0 else ("#8b1a1a" if ci == 2 else "#333333")
                ax.text(
                    cx + 0.005,
                    y_pos,
                    cell,
        ...
        ax.set_ylim(0, 1)
        handles = [
            mpatches.Patch(fc=v, ec="#cccccc", lw=0.5, label=k) for k, v in SEC_COLORS.items()
        ]
        ax.legend(
            handles=handles,
            loc="upper center",
        ...
        path = os.path.join(_sim_fig_dir(seed), "fig8_sim%04d_parametros.png" % seed)
        fig.savefig(path, dpi=dpi)
        plt.close(fig)
        print("  -> %s" % path)
    return path
...
def plot_fig9_mapa_pips_balsas(membrane, dpi=300):
    ...
    cmap_bg = mcolors.LinearSegmentedColormap.from_list(
        "bg_neutral", ["#bdd7e7", "#dce8e4", "#ece8de", "#ddd0b8", "#c4b090"], N=256
    )
    ...
    with plt.rc_context(PUB_RC):
        ...
        # PIPs individuales
        pip_handles = []
        for sp, (xs, ys) in sorted(pip_by_sp.items(), key=lambda kv: -len(kv[1][0])):
            ...
            pip_handles.append(
                mpatches.Patch(fc=col, ec="black", lw=0.4, label="%s  (n = %d)" % (sp, len(xs)))
            )
        ...
        _info_box(ax, "%d PIPs · %d especie(s)" % (n_pips, len(pip_by_sp)), loc="lower right")

        path = os.path.join(_sim_fig_dir(seed), "fig9_sim%04d_mapa_pips_balsas.png" % seed)
...
def plot_all_figures(membrane, vol=None, labels=None, stats=None, dpi=300):
    ...
    from simulacion.model_3d import build_physical_volume

    if vol is None or labels is None or stats is None:
        vol, labels, stats = build_physical_volume(membrane, bins_xy=55, bins_z=80)
