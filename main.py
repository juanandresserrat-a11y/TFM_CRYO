...
# Ruta específica del servidor del máster (ajustar según entorno si es necesario)
...
from exportacion.export import export_training
...
def parse_args():
    ...
    p.add_argument(
        ...
        default=[27, 42],
        metavar="N",
        help="Numeros de simulacion (default: 27 42)",
    )
    p.add_argument("--size", type=float, nargs=2, default=[50.0, 50.0], metavar=("X", "Y"))
    p.add_argument(
        "--paraview", action="store_true", help="VTP + PDB en CryoET/paraview/simulacion{N}/"
    )
    p.add_argument(
        "--model3d",
        action="store_true",
    ...
    p.add_argument(
        "--mrc", action="store_true", help="MRC simplificado + doble gaussiana para PolNet"
    )
    p.add_argument(
        "--positions", action="store_true", help="PDB completo + CSV + PolNet particle list"
    )
    p.add_argument(
        "--validate",
        action="store_true",
    ...
    p.add_argument(
        "--stats", action="store_true", help="Estadisticas del dataset (requiere >1 simulacion)"
    )
    p.add_argument("--dpi", type=int, default=200)
    p.add_argument("--all", action="store_true", help="Ejecuta todo")
    return p.parse_args()
...
def run_sim(seed, size_nm, args):
    ...
    if args.paraview or args.all:
        from exportacion.export_paraview import export_all_paraview

        export_all_paraview(b)

    if args.model3d or args.all:
        from simulacion.model_3d import export_physical_model_mrc, plot_physical_model

        export_physical_model_mrc(b)
        plot_physical_model(b)

    if args.mrc or args.all:
        from exportacion.export_mrc import (
            export_double_gaussian_mrc,
            export_label_mrc_with_closing,
            export_mrc,
            generate_polnet_yaml,
        )
    ...
    if args.positions or args.all:
        from exportacion.export_positions import export_all_positions

        export_all_positions(b)

    if args.figures or args.all:
        from visualizacion.figures import plot_all_figures
    ...
    if args.validate or args.all:
        from analisis.validation import (
...
def main():
    ...
    print("BicapaCryoET  %d simulacion(es)  %.0fx%.0f nm" % (len(sims), size_nm[0], size_nm[1]))

    for seed in sims:
        run_sim(seed, size_nm, args)

    if args.stats and len(sims) > 1:
        from visualizacion.dataset_stats import compute_dataset_stats, plot_dataset_summary

        stats = compute_dataset_stats(sims, size_nm=tuple(size_nm), run_validation=args.validate)
