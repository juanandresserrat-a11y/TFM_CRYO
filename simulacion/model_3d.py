...
import mrcfile
import numpy as np
from analisis.analysis import midplane_map
from config import OUTPUT_DIR
from simulacion.electron_density import ELECTRON_DENSITY, LIPID_ED_HEADGROUP, LIPID_ED_TAIL
...
def _tail_density_with_unsaturation(lipid_name: str, z_frac: float) -> float:
    ...
    if lipid_name == "CHOL":
        if z_frac <= 0.75:
            ...
            t = (z_frac - 0.75) / 0.25  # 0→1 en el ultimo 25%
            return float(0.308 - t * 0.023)
    ...
    if ndb > 0 and dbpos is not None and nc > 0:
        z_db = dbpos / nc
        for _ in range(ndb):
            penalty = UNSATURATION_PENALTY * np.exp(-0.5 * ((z_frac - z_db) / 0.15) ** 2)
            base_ed -= penalty
...
def build_physical_volume(
    ...
    for l in todos:
        ...
        if l.tail1 and len(l.tail1) >= 2:
            for seg_i in range(len(l.tail1) - 1):
                ...
                from simulacion.electron_density import ED_TAIL_MAX, ED_TAIL_MIN

                _wt_raw = 0.30 + 0.20 * (ed_tail_local - ED_TAIL_MIN) / (ED_TAIL_MAX - ED_TAIL_MIN)
                wt_tail = float(np.clip(_wt_raw, 0.28, 0.52))

                iz_seg = np.argmin(np.abs(z_centers - z_mid_seg))
                for diz in range(-1, 2):
                    iz2 = np.clip(iz_seg + diz, 0, bins_z - 1)
                    wt = np.exp(-0.5 * (diz * dz_nm / 0.25) ** 2) * wt_tail
                    if wt > 0.05:
                ...
                for diy in range(-r_vox, r_vox + 1):
                    if dix**2 + diy**2 <= r_vox**2:
                        ...
                        for iz in range(bins_z):
                            vol[ixx, iyy, iz] = vol[ixx, iyy, iz] * 0.3 + ED_PROTEIN * 0.7
    ...
    stats = {
        ...
        "n_protein_objects": (
            len(membrane.perturbations) if hasattr(membrane, "perturbations") else 0
        ),
        "contrast_Lo_Ld": (
            float(lo_voxels.mean() - ld_voxels.mean())
            if lo_voxels.size and ld_voxels.size
            else 0.0
        ),
...
def export_physical_model_mrc(
    ...
    print(
        "  -> modelo3d/volumenes/bicapa_fisica_simulacion%04d.mrc  %dx%dx%d voxeles  %.0f A/voxel"
        % (membrane.seed, bins_xy, bins_xy, bins_z, voxel_angstrom)
    )
    print(
        "  cabezas: %.3f e/A3  |  Lo: %.3f  |  Ld: %.3f  |  CHOL: %.3f  |  contraste Lo/Ld: %.4f e/A3"
        % (
            stats["ed_head_mean"],
            stats["ed_tail_Lo"],
            stats["ed_tail_Ld"],
...
def plot_physical_model(
    ...
    with plt.rc_context(PLT):
        ...
        fig.suptitle(
            ...
            "Densidad electronica con insaturaciones, fases Lo/Ld y PIPs" % membrane.seed,
            fontsize=12,
            fontweight="bold",
        )
        ...
        ax = axes[0, 0]
        slc_xz = vol[:, mid_y, :]
        im = ax.imshow(slc_xz.T, origin="lower", cmap="gray", extent=ext_xz, aspect="auto")
        plt.colorbar(im, ax=ax, shrink=0.85, label="$e \\cdot \\AA^{-3}$")
        ax.axhline(
            z_head_ext_nm, color="#2dc653", lw=1.2, ls="--", alpha=0.85, label="Cabezas ext"
        )
        ax.axhline(
            z_head_int_nm, color="#fb8500", lw=1.2, ls="--", alpha=0.85, label="Cabezas int"
        )
        ax.axhline(0.0, color="#adb5bd", lw=0.8, ls=":", alpha=0.7, label="Plano medio")
        ax.set_xlabel("X (nm)")
        ax.set_ylabel("Z relativo (nm)")
        ax.set_title("Slice XZ — Patron dark-bright-dark\ncabezas (oscuro) + nucleo (claro)")
        ax.legend(fontsize=7.5, loc="upper right")

        ax = axes[0, 1]
        slc_yz = vol[mid_x, :, :]
        im2 = ax.imshow(slc_yz.T, origin="lower", cmap="gray", extent=ext_yz, aspect="auto")
        ...
        ax.fill_betweenx(
            z_centers, ELECTRON_DENSITY["water"], ed_profile, alpha=0.35, color="#d4a017"
        )
        ax.axhline(
            z_head_ext_nm,
            color="#2dc653",
        ...
        from analisis.analysis import raft_fraction_map

        raft_map = raft_fraction_map(membrane, membrane.outer_leaflet, bins=bins_xy)
        im3 = ax.imshow(head_proj.T, origin="lower", cmap="gray", extent=ext_xy, aspect="equal")
        plt.colorbar(im3, ax=ax, shrink=0.85, label="ED max ($e \\cdot \\AA^{-3}$)")
        cs = ax.contour(
            np.linspace(0, Lx, bins_xy),
        ...
        ax = axes[1, 1]
        from analisis.analysis import order_parameter_map, pip_density_map
        ...
        patches_leyenda = [
            ...
            mpatches.Patch(
                facecolor="#9b5de5", label="PIPs (%.3f $e \\cdot \\AA^{-3}$)" % stats["ed_pip"]
            ),
            mpatches.Patch(
                facecolor="#d4a017", label="CHOL (%.3f $e \\cdot \\AA^{-3}$)" % stats["ed_chol"]
            ),
            mpatches.Patch(
                facecolor="#f0f4ff",
                edgecolor="#aaa",
            ...
            mpatches.Patch(
                ...
                % (stats.get("n_protein_objects", 0), stats.get("ed_protein", 0.400)),
            ),
        ]

        fig.legend(
            handles=patches_leyenda,
            loc="lower center",
            ncol=4,
            fontsize=8.5,
            frameon=True,
