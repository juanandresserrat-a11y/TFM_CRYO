...
def electron_density_profile(
    ...
    for l in todos:
        ...
        # cabeza polar
        iz_head = np.clip(np.searchsorted(z_edges, z_h) - 1, 0, bins_z - 1)
        ...
        # glicerol
        iz_glyc = np.clip(np.searchsorted(z_edges, z_g) - 1, 0, bins_z - 1)
...
def electron_density_volume(
    ...
    for l in todos:
        ...
        for iz, zc in enumerate(z_centers):
            ...
            if w_head > 0.05:
                vol[ix, iy, iz] = vol[ix, iy, iz] * (1 - w_head) + ed_head * w_head
            ...
            if l.tail1 and len(l.tail1) > 2:
                ...
                if z_tail_lo <= zc <= z_tail_hi:
                    vol[ix, iy, iz] = vol[ix, iy, iz] * (1 - w_tail) + ed_tail * w_tail
