...
def _plot_strip_r6(ax, values, x_center, color,
    ...
    ax.errorbar(x_center, mu, yerr=sem, fmt="none",
                color=C["line"], capsize=5, capthick=1.6,
                elinewidth=1.6, zorder=5)

    jitter = np.linspace(-0.12, 0.12, len(values))
    for ji, vi in zip(jitter, values):
        ax.scatter(x_center + ji, vi, s=55,
    ...
    with plt.rc_context(PUB_RC):
        ...
        for i, lf in enumerate(leaflets):
            ...
            if remainder > 0.5:  # solo si hay fraccion significativa
                ax_comp.barh(y_pos[i], remainder, left=left[i],
                             color=RESTO_COLOR, height=0.28,
                             edgecolor="white", linewidth=0.35)
                if remainder > 3.0:
                    ax_comp.text(left[i] + remainder / 2, y_pos[i],
                                 f"{remainder:.0f}%", ha="center", va="center",
                                 fontsize=7.5, color="white", fontweight="bold")
        ...
        ax_comp.legend(handles=handles_full, loc="center",
    ...
    with plt.rc_context(PUB_RC):
        ...
        for i, (label, key, bench, ref, pass_key) in enumerate(rows):
            ...
            ax_tbl.text(x_positions[1], y, val_str, transform=ax_tbl.transAxes,
                        fontsize=8.5, va="center", color=C["line"], ha="center")
            ax_tbl.text(x_positions[2], y, ref, transform=ax_tbl.transAxes,
                        fontsize=8, va="center", color=C["neutral"], ha="center")
...
def plot_R3_organizacion(membrane: BicapaCryoET, dpi: int = 300) -> str:
    ...
    with plt.rc_context(PUB_RC):
        ...
        ax_e.text(0.03, 0.97,
                  ...
                  transform=ax_e.transAxes, fontsize=8.5,
                  va="top", bbox=dict(fc="white", ec=C["neutral"],
                                     lw=0.5, pad=2.5, alpha=0.9))
...
def plot_R4_campos(membrane: BicapaCryoET, dpi: int = 300) -> str:
    ...
    campos = {
        ...
        "c2  Rugosidad ext.\n(Å)":      analysis.roughness_map(membrane, membrane.outer_leaflet, bins=bins),
        "c3  Rugosidad int.\n(Å)":      analysis.roughness_map(membrane, membrane.inner_leaflet, bins=bins),
        "c4  Raft ext.\n[0,1]":           analysis.raft_fraction_map(membrane, membrane.outer_leaflet, bins=bins),
        "c5  Raft int.\n[0,1]":           analysis.raft_fraction_map(membrane, membrane.inner_leaflet, bins=bins),
...
def plot_R6_multisimulacion(stats: Dict, dpi: int = 300) -> str:
    ...
    with plt.rc_context(PUB_RC):
        ...
        ax_D.legend(loc="lower right", frameon=True, framealpha=0.9,
                    edgecolor=C["neutral"], fontsize=8.5)
        _light_grid(ax_D)
        _panel_label(ax_D, "D")
